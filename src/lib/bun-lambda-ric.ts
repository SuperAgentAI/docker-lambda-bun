...
// Add direct mode flag
const args = process.argv.slice(2)
const directMode = args.includes('--direct-mode') || args.includes('-d')
...
async function init(): Promise<Lambda> {
  ...
  const handlerPath = `${env('LAMBDA_TASK_ROOT')}/${handlerName}`

  // Check if the handlerName points to an executable file
  try {
      ...
      return {
        fetch: async (request: Request) => {
          const result = Bun.spawnSync(
            [handlerPath, JSON.stringify(request.json())],
            {
          ...
          if (result.exitCode !== 0) {
            console.error(
              'Executable handler failed:',
              // @ts-ignore
              result.stderr?.toString() || 'No error output',
            )
            throw new Error(
              `Executable handler failed with code ${result.exitCode}`,
            )
          }
          return Promise.resolve(
            // @ts-ignore
            new Response(result.stdout?.toString() || '', { status: 200 }),
          )
        },
      }
    }
    ...
    console.warn(
      `Handler ${handlerName} is not an executable, proceeding to load as module.`,
    )
  }

  // Otherwise, attempt to load as a module
  ...
  // Modify the fetch function to handle direct mode
  ...
  // Wrap the fetch function to handle direct mode
  module.fetch = async (request: Request, server: Server) => {
    if (directMode) {
      // In direct mode, extract the raw event from the request
      const rawEvent = await request.json()
      // Call the handler directly with the raw event
      // @ts-ignore
      const result = await originalFetch(rawEvent.event, rawEvent)
      // Return the result wrapped in a Response
      return new Response(JSON.stringify(result), {
        headers: { 'Content-Type': 'application/json' },
      })
    }
    // Normal mode - call the original fetch
    return originalFetch(request, server)
  }

  if (module.websocket === undefined) {
    return module
  }

  for (const name of ['open', 'message', 'close']) {
    const method = module.websocket[name]
    if (method === undefined) {
      continue
    }
...
async function receiveRequest(): Promise<LambdaRequest> {
  ...
  if (traceId === undefined) {
    console.warn('Runtime received a request without a trace ID')
  }
...
class LambdaServer implements Server {
  ...
  async accept(request: LambdaRequest): Promise<unknown> {
    if (directMode) {
      // In direct mode, forward the raw event directly to the handler
      return this.#lambda.fetch(
        new Request('https://lambda/', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'x-amzn-requestid': request.requestId,
            'x-amzn-trace-id': request.traceId,
            'x-amzn-function-arn': request.functionArn,
            ...(request.deadlineMs
              ? { 'x-amzn-deadline-ms': request.deadlineMs.toString() }
              : {}),
          },
...
export const run: LambdaRuntime = async (cwd: string, handler: string) => {
  console.log(
    `Initializing native bun lambda runtime in ${cwd} with handler ${handler}`,
  )
